  ...
  <SectionWrapper className="relative overflow-hidden">
    ...
    <div className="relative grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {industries.map((ind, index) => {
        ...
        return (
          <Reveal key={ind.id} delay={(index % 5) * 70}>
            <Link to={`/industries#${ind.slug}`} className="group relative block h-full overflow-hidden rounded-[1.35rem] border border-border bg-white p-5 text-left shadow-[0_12px_30px_rgba(15,23,42,0.06)] transition-all duration-500 hover:-translate-y-1 hover:border-accent/30 hover:shadow-[0_20px_45px_rgba(15,23,42,0.12)]">
